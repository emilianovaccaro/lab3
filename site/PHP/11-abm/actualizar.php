<?php
    ...
    try {
        ...
        if (isset($_FILES["imagen"]) && $_FILES["imagen"]["name"] !== '') {
            ...
            $sql = "UPDATE automoviles SET Marca = :marca, Modelo = :modelo, Precio = :precio, Anio = :anio, Imagenes = :pdfData WHERE ID = :id";
            $stmt = $dbh->prepare($sql);
            $stmt->bindParam(':pdfData', $pdfData, PDO::PARAM_LOB);
        } else {
            $movimientoDeLaRequest = $movimientoDeLaRequest . "\nFile nuevo NO seleccionado";
            $sql = "UPDATE automoviles SET Marca = :marca, Modelo = :modelo, Precio = :precio, Anio = :anio WHERE ID = :id";
            $stmt = $dbh->prepare($sql);
        }
